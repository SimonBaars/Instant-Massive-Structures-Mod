...
- **LiveBoat path: PASS (short +Z loop subset)** — Legacy path data clear (`EventHandler.getAnimationFor`: board → N×`{0,0,+1}` @300ms → stop; dialog sets N). Ported as path mover: **4 frames** `LiveBoat0..3` (9×8×29, ~2088 blocks), **6 ticks/step**, boarding **40 ticks (~2s)** vs legacy 10s, default distance **24**, playtested **`/imsm live boat 16`** at (900,68,-200). Log: start → depart → complete short loop → reboard → depart (multiple loops). Chat: "SimJoo's Maritime Solutions… 16 blocks (+Z), 4 frames, 6 ticks/step, short loop on." `Live_Bus` wired same path (1 frame `Live_Bus0`). `Live_Bus2` wired same path (dedicated frame `Live_Bus20`, distinct schematic vs `Live_Bus0`). Chat-typed distance dialog **deferred** (command/default distance instead). Collision-explode + legacy `removeStuff` trail strips simplified to full-bounds clear each step. Screenshots: `36-boat-started.png` (maritime chat + hull), `37-boat-sailing.png`, `38-boat-mid.png`.

- **Live_Bus2 path: PASS (wire + board subset)** — Legacy `Live_Bus2` nslides=1 → dedicated `Live_Bus20` (5×7×16, 560 blocks; md5 ≠ `Live_Bus0`). Same `EventHandler` +Z animation as Live_Bus/LiveBoat. Ported as path mover with `BOAT_BUS_PATH`; `/imsm live bus2 16`. Log: Bus Depot chat → place Live_Bus20 → depart. Pad fill missed (chunk unload) → early obstacle-explode; full cruise film N/A. Screenshots: `58-bus2-boarding.png`, `59-bus2-mid.png`.
...
| Live_Bus2 | **DONE subset**: same +Z `BOAT_BUS_PATH` as Live_Bus; dedicated frame `Live_Bus20` (distinct schematic); `/imsm live bus2 [n]` |
...
- Wire Live_Bus2 dedicated Live_Bus20 + same +Z PathMotion + bus2shot + PORT_STATUS
