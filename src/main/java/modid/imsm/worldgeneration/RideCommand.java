	 ...
	 // Minecraft.getMinecraft().thePlayer.addChatMessage(new TextComponentString("Structure riding went broken in the 1.9 upgrade... Will fix asap!"));
	  ...
	  for(int i = 0; i<IMSM.eventHandler.liveCreators.size(); i++){
			...
			if(IMSM.eventHandler.liveCreators.get(i).structureName.equals("Live_FerrisWheel") && IMSM.eventHandler.liveCreators.get(i).closeTo(5, Minecraft.getMinecraft().player.posX, Minecraft.getMinecraft().player.posY, Minecraft.getMinecraft().player.posZ, struct.width, struct.height, struct.length) && IMSM.eventHandler.liveCreators.get(i).ride==null){
				...
				Minecraft.getMinecraft().player.sendMessage(new TextComponentString("Please hop aboard in the bottommost cart."));
				return;
				...
				IMSM.eventHandler.isRiding=IMSM.eventHandler.liveCreators.get(i);
				Minecraft.getMinecraft().player.sendMessage(new TextComponentString("We'll pick you up on our next ride! Please hop aboard then."));
				return;
			}
		}
	  Minecraft.getMinecraft().player.sendMessage(new TextComponentString("There is no freefall or ferris wheel is sight. Please step closer."));
	  } else {
		  for(int i = 0; i<IMSM.eventHandler.liveCreators.size(); i++){
