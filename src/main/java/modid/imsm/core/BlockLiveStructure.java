...
public class BlockLiveStructure extends Block {
	...
	if(worldIn.isRemote) {   
  	...
  	if(IMSM.eventHandler.liveCreators.size()>4){
  		Minecraft.getInstance().thePlayer.addChatMessage(new TextComponentString("You have now placed more than 5 Live Structures, which may cause some lag."));
  		Minecraft.getInstance().thePlayer.addChatMessage(new TextComponentString("Do the command /removelive to remove all live structures"));
  	}
  	if(name.equals("LivePlane") || name.equals("LiveFlyingShip1") || name.equals("LiveFlyingShip2") || name.equals("LiveAirplane") || name.equals("LiveAirBalloon") || name.equals("Live_Flying_Helicopter")){
  		Minecraft.getInstance().thePlayer.addChatMessage(new TextComponentString("Thanks for choosing SimJoo's Aviation Solutions."));
  		Minecraft.getInstance().thePlayer.addChatMessage(new TextComponentString("Please type in the distance (in blocks) you'd like to fly."));
  		IMSM.dialoge=1;
  		IMSM.currentInput = IMSM.eventHandler.liveCreators.get(IMSM.eventHandler.liveCreators.size()-1);
  	} else if (name.equals("LiveBoat")){
  		Minecraft.getInstance().thePlayer.addChatMessage(new TextComponentString("Thanks for choosing SimJoo's Maritime Solutions."));
  		Minecraft.getInstance().thePlayer.addChatMessage(new TextComponentString("Please type in the distance (in blocks) you'd like to sail."));
  		IMSM.dialoge=2;
  		IMSM.currentInput = IMSM.eventHandler.liveCreators.get(IMSM.eventHandler.liveCreators.size()-1);
  	} else if(name.equals("Live_Bus") || name.equals("Live_Bus2")){
  		Minecraft.getInstance().thePlayer.addChatMessage(new TextComponentString("Thanks for choosing SimJoo's Bus Depot."));
  		Minecraft.getInstance().thePlayer.addChatMessage(new TextComponentString("Please type in the distance (in blocks) you'd like to ride."));
  		IMSM.dialoge=3;
  		IMSM.currentInput = IMSM.eventHandler.liveCreators.get(IMSM.eventHandler.liveCreators.size()-1);
  	} else if (name.equals("Live_FerrisWheel") || name.equals("Live_Fair_FreeFall")){
  		Minecraft.getInstance().thePlayer.addChatMessage(new TextComponentString("Use /ride to ride this structure!."));
  	}
        return true;
    }
